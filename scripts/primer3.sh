...
sed 's/SEQUENCE_TARGET=/SEQUENCE_TARGET='"$TARGET"'/' > $OUTDIR/primer3_settings.txt
...
$PRIMER3_PATH $OUTDIR/primer3_settings.txt --error="$OUTDIR"/"$ID".err --output="$OUTDIR"/"$ID".out

rm $OUTDIR/primer3_settings.txt
